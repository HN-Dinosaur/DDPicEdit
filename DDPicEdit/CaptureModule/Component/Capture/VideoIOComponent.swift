...
final class VideoIOComponent: DeviceIOComponent {
    ...
    private let workQueue = DispatchQueue(label: "org.DDPICEDIT.DispatchQueue.VideoCapture")
...
extension VideoIOComponent {
    ...
    func switchCamera(session: AVCaptureSession) {
        ...
        do {
            try self.setupInput(session: session)
            self.setupOutputConnection(photoOutput)
            self.setupOutputConnection(videoOutput)
        } catch {
            _print(error)
        }
    }
...
extension VideoIOComponent {
    ...
    var zoomFactor: CGFloat {
        return self.device?.videoZoomFactor ?? 0
    }
